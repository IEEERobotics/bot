...
class MecDriver(driver.Driver):
    ...
    def angle(self):
        ...
        """
        # Combine wheel velocity vectors,
        # return angle (corrected to use forward as zero)
        # Note: Mecannum wheels must be oriented as per http://goo.gl/B1KEUV
        v_forward_right = self.motors["front_left"].velocity +\
            self.motors["back_right"].velocity
        v_forward_left = self.motors["front_right"].velocity +\
        ...
        return int(round(degrees(atan2(v_forward_right, v_forward_left)
                    - pi / 4))) % 360
        # TODO verify math; -pi/4 is because hypot
        # will compute direction along forward_right diagonal
        # TODO correct this so that zero angle is returned (instead of -45)
        # even when speed is near-zero
    ...
    def rotate(self, rotate_speed):
        ...
        # Validate params
        assert MecDriver.min_rotate_speed <=\
                    rotate_speed <=\
