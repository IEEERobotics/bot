...
while True:
    ...
    # print "[{:0.3f}] Voltage: {:0.2f}, Current: {:3d} | {:3d},
    #    Position: {:6d} | {:6d}, Velocity: {:3d} | {:3d}".format(
    #       time()-t0, dmcc.voltage, left.current, right.current,
    #       left.position, right.position, left.velocity, right.velocity)
    print "{:7.3f} DMCC : Voltage: {:0.2f}".format(time()-t0, gun_dmcc.voltage)
    print "       Left : Current: {:3d}, Position: {:6d}, Velocity: {:3d}, \
            Calc_vel: {:0.1f}".format(
        left.current, left_pos, left.velocity, left_delta/period)
    print "       Right: Current: {:3d}, Position: {:6d}, Velocity: {:3d}, \
            Calc_vel: {:0.1f}".format(
