...
def getHorizLaunchAngle(Xpos, Ypos):
    """Calculate the horizontal (pan) angle to the target,
    from X & Y coordinates"""
...
def getMinElevationAngle(distance):
    """Calculate the minimum launch angle, geometrically,
...
def getVertLaunchAngle(V, Theta, z, targetDistance):
    """Calculate the vertical (tilt) launch angle, distance
    ...
    while t <= Tmax:
        ...
        # print "X: {0:.3f} Z:{1:.3f} Vx:{2:.4f} Vz:{3:.4f}".format(
        # x, z, Vx, Vz)
...
def getFiringSolution(Xpos, Ypos, offset, velocity):
    ...
    if velocity < 1:
        ...
        raise ValueError
    V = velocity
    targetDistance = getTargetDistance(Xpos, Ypos)
    # horizontal distance to target in m
    horizDeflection = getHorizLaunchAngle(Xpos, Ypos)
    # pan angle in degrees
    elevAngle = getMinElevationAngle(targetDistance)
    # lower bounds angle in degrees
    ...
    # calculate servo positions based on desired launch
    # angle and bot orientation
...
def getServoAngle(elevAngle):
    """Returns the servo position required for actuation to
    set a given vertical launch angle """
