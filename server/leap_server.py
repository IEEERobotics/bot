    ...
    def handle(self):
        ...
        while True:
            if COMMAND_PROMPT is not None:
                self.wfile.write(COMMAND_PROMPT)  # self.request.sendall(COMMAND_PROMPT)
            
            data = self.rfile.readline().strip()
            if not data or data.startswith('\x04'):
                break  # client has quit or EOF received, nothing more to do here
        ...
        if controlServer.driver.speed > 0:
            controlServer.driver.move(0, 0)  # stop bot, in case still moving
